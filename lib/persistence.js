...
async function _load() {
  ...
  try {
    const [inventory, orders, sales, duties, competencies, settings, txLog, staff, recipes] = await Promise.all([
    ...
    if (staff)        store.staff        = JSON.parse(staff);
    if (recipes)      store.recipes      = JSON.parse(recipes);
    if (txLog) {
